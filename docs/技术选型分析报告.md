...
> **⚠️ 重要更新（2026-07-17）**
>
> 本文档的部分技术决策已被 **ADR-0001** 取代：
> - **React Web界面** → 已被 **TUI** 替代（参见 `docs/adr/0001-采用TUI替代React-Web界面.md`）
> - **FastAPI/WebSocket必需** → 改为按消费者契约保留的可选适配层
> - **LangGraph强制使用** → 细化为实现手段非验收条件
>
> 本文档保留作为历史决策记录。当前有效决策请参考 ADR-0001。
>
> **决策优先级**: ADR-0001 > 本文档
...
✅ **混合技术栈**: Python后端 + TypeScript前端 ⚠️ _已部分取代（见ADR-0001）_  
✅ **后端框架**: FastAPI + LangGraph ⚠️ _FastAPI改为可选适配层（见ADR-0001）_  
~~✅ **前端框架**: React + TypeScript~~ ❌ _已取代：改用TUI（见ADR-0001）_  
✅ **扩展模式**: Ruflo风格插件系统  
~~✅ **架构风格**: API优先，前后端分离~~ ⚠️ _已细化：TUI为主，API为可选适配层（见ADR-0001）_
